...
export default async () => {
  try {
    ...
    users.map((user) => senUssd(user));
  } catch (e) {
    console.error(e);
  }
};

const senUssd = async (user) => {
  try {
    ...
    while (pendingUssd.length !== 0) {
      ...
      await waitTimeForSendUssd(device.token, pendingUssd[0], seconds);
...
const waitTimeForSendUssd = (deviceToken, pendingUssdZero, seconds) => {
